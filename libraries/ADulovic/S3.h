...
=========1=========2=========3=========4=========5=========6=========7========*/
void loopS3(){
	if( millis() - tS3 > iS3*1000 ) {
		tS3 = millis();        			// save the last time this fired
		Serial.print("Temperature = "); Serial.print(bmp.readTemperature()); Serial.println(" C");
		send(msgS3T.set( bmp.readTemperature(),2 ));
		Serial.print("Pressure = "); Serial.print(bmp.readPressure()/100); Serial.println(" Pa");
		send(msgS3B.set( bmp.readPressure()/100,2 ));
		Serial.println();
	}
}
